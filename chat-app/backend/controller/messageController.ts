...
export async function getAllConversationController(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        res.status(200).json({
            success: true, // Fixed string 'true' to boolean true
            data: result
        });
    } catch (error) {
        next(error);
...
export async function sendMessageController(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        // Use null fallback if string is missing or empty whitespace
        ...
        // 🚀 CRITICAL CONTROLLER FIX: Validate AFTER computing file values
        if (!text && !image && !voice && !video) {
            throw new AppError('message required', 400);
        }
        
        const result = await sendMessageService(Number(senderId), Number(receiverId), { 
            text, 
            image, 
            voice, 
            video 
...
export async function updateMessageController(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        res.status(200).json({
            success: true, // Fixed typo text 'succes'
            data: result
        });
    } catch (error) {
        next(error);
