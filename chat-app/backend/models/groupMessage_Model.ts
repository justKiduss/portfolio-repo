...
const groupMessageModel=()=>{
    ...
    return {
        ...
        getById: async (id:number)=>{
            const res=await pool.query(`SELECT * from groupMessage WHERE id=$1`,[id]);
            return res.rows[0];
        },
        create:async (sender_id:number,group_id:number,data:sendMessageDTO)=>{
            const {text,image,video,voice}=data;

            const res=await pool.query(`INSERT INTO groupMessage (sender_id,group_id,text,image,voice,video) VALUES (
                $1,$2,$3,$4,$5,$6) RETURNING id,sender_id,group_id,text,image,video,voice,created_at`,
            [sender_id,group_id,text,image,voice,video]);
            return res.rows[0];
        },
        update:async (id:number,data:updateMessageDTO)=>{
            const {text,image,video,voice}=data;
            const res=await pool.query(`UPDATE groupMessage SET text=$1,image=$2,video=$3,voice=$4 where id=$5 RETURNING *`,
                [text,image,video,voice,id]
            );
            return res.rows[0];
        },
        delete:async (id:number,sender_id:number)=>{
            const res=await pool.query(`DELETE from groupMessage WHERE id=$1 AND sender_id=$2 RETURNING *`,[id,sender_id]
            );
            return res.rows[0]; 
        },
